...
void UserArguments::parse(int argc, char** argv)
{
    try
    {
        ...
        TCLAP::ValueArg<Physics::Time> time_of_simulation_arg("t", "time","total simulation time", false, TIME_OF_SIMULATION, "double", cmd);
        TCLAP::ValueArg<unsigned int> number_neurons_arg("n", "neurons", "total number of neurons to consider for the simulation", false, NUMBER_NEURONS, "int", cmd);
        ...
        TCLAP::ValueArg<Physics::Time> time_step_arg("d", "dt", "time step of the simulation", false, TIME_STEP, "Time", cmd);
